"""HashKey Chain RPC connector: read balance, recent txs."""
try:
...
def get_balance(address: str) -> dict:
    """Get native token balance for address. Returns dict with wei and eth."""
    w3 = get_web3()
    if not w3 or not address or address == "0x0000000000000000000000000000000000000000":
        return {"wei": "0", "eth": "0.0", "connected": False}
    try:
        addr = Web3.to_checksum_address(address)
        wei = w3.eth.get_balance(addr)
        eth = float(Web3.from_wei(wei, "ether"))
        return {"wei": str(wei), "eth": f"{eth:.6f}", "connected": True}
    except Exception as e:
        return {"wei": "0", "eth": "0.0", "connected": False, "error": str(e)}
